...
def test_resolve_nnunet_pred_with_zero_padded_numeric_variant(tmp_path: Path) -> None:
    nn_root = tmp_path / "nnunet"
    nn_root.mkdir()

    label_file = tmp_path / "PROSTATE_102.nii.gz"
    image_file = tmp_path / "PROSTATE_102_0000.nii.gz"

    expected = nn_root / "PROSTATE_0102.nii.gz"
    expected.write_text("x", encoding="utf-8")
